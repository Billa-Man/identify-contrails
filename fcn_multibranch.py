...
class FCN_Multibranch(nn.Module):
    def __init__(self, num_channels, num_classes):
        super(FCN_Multibranch, self).__init__()

        self.individual_models = nn.ModuleList([FCN(1) for _ in range(num_channels)])
        self.combine = nn.Conv2d(num_channels * 64, 128, kernel_size=1)
        self.final_layer = nn.Linear(128, num_classes)

    def forward(self, x):

        individual_outputs = [model(x[:, i:i+1, :, :]) for i, model in enumerate(self.individual_models)]
        combined_output = torch.cat(individual_outputs, dim=1)
        combined_output = self.combine(combined_output)
